    ...
    <div class="grid grid-cols-1 gap-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-6 items-start">
                <!-- Penyesuaian pengecekan poster agar sinkron dengan welcome.blade -->
                @if($event->poster_path)
                    <img src="{{ asset('posters/' . $event->poster_path) }}" alt="{{ $event->title }}" class="w-24 h-24 rounded-2xl object-cover">
                @else
                    <div class="w-24 h-24 bg-indigo-50 flex items-center justify-center text-indigo-200 rounded-2xl">
                ...
                <div>
                    ...
                    <!-- Perbaikan error format tanggal di baris ini -->
                    <p class="text-slate-500">{{ \Carbon\Carbon::parse($event->event_date)->translatedFormat('d F Y') }} • {{ $event->location }}</p>
            ...
            <form action="{{ route('checkout.store', $event->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="tel" name="customer_phone" placeholder="08xxxxxxxx" class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required value="{{ old('customer_phone') }}">
                    </div>
                </div>
